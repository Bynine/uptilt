...
public class MoveList_Kicker extends MoveList{
	...
	public Move slide(Fighter user) { 
		...
		m.actionList.addVelocityChange(user, 2, user.direct() * 10, Action.ChangeVelocity.noChange);
		Hitbox early = new Hitbox(user, 3, 2, 4, Hitbox.SAMURAIANGLE, 12, 0, 18);
		Hitbox late = new Hitbox(user, 3, 1, 3, 80, 18, 0, 12);
		late.connect(early);
		m.actionList.addActionCircle(early, 4, 12);
		m.actionList.addActionCircle(late, 13, 20);
		return m;
	}
	...
	public Move nWeak(Fighter user) {
		...
		m.actionList.addActionCircle(h1, 4, 7);
		return m;
	}
	...
	public Move uWeak(Fighter user) {
		...
		m.actionList.addActionCircle(h1, 0, 10);
		m.actionList.addActionCircle(h2, 10, 20);
		m.actionList.addActionCircle(h3, 20, 30);
		return m;
	}
	...
	public Move dWeak(Fighter user) {
		...
		late.connect(early);
		m.actionList.addActionCircle(early, 6, 8);
		m.actionList.addActionCircle(late, 9, 12);
		return m;
	}
	...
	public Move sCharge(Fighter user) {
		...
		Hitbox h1 = new Hitbox(user, 3, 6, 16, Hitbox.SAMURAIANGLE, 20, 0, 20, c);
		m.actionList.addActionCircle(h1, 20, 25);
		return m;
	}
	...
	public Move dCharge(Fighter user) {
		...
		Effect.Charge c = new Charge(3, 23, 0.09f, user, m);
		m.actionList.addCharge(user, c);
	...
	public Move fAir(Fighter user) {
		Move m = new Move(user, 30);
		Hitbox early = new Hitbox(user, 5, 6, 15, 30, 12, 0, 20);
		Hitbox late = new Hitbox(user, 3, 3, 5, 50, 12, 0, 15);
		early.setProperty(Hitbox.Property.ELECTRIC);
		late.connect(early);
		m.actionList.addActionCircle(early, 8, 12);
		m.actionList.addActionCircle(late, 13, 16);
		return m;
	}
	...
	public Move airGrab(Fighter user) {
		Move m = new Move(user, 20);
		m.helpless = true;
		m.actionList.addVelocityChange(user, 4, user.direct() * 4, 1);
		Grabbox g1 = new Grabbox(user, 8, 0, 10);
